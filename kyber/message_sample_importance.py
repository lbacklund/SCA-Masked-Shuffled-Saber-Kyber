...
model_folder = "models/message/25k/"        
data_folder = "traces/profiling/"
model_name = "kyber_message_*.h5"
################################################### 
...
def load_models(model_folder,model_file):
    ...
    for file_name in os.listdir(model_folder):
        if fnmatch.fnmatch(file_name, model_file):
            if not int(file_name[14:15]) < 5: continue
...
def load_traces(num_of_traces):
    ...
    traces = np.load(data_folder+'cut_joined_traces.npy', mmap_mode="r")[25000*253:25000*253+num_of_traces, -225*2:-225]
    labels = np.load(data_folder+'cut_joined_message_labels.npy', mmap_mode="r")[25000*253:25000*253+num_of_traces]
...
models = load_models(model_folder+"bit_in_byte_0/", model_name)
...
for i, model in enumerate(models):
    ...
    # Save plots
    plt.savefig(f"sample_importance/message_bit0_sample_importance_{i}.png")
...
np.save("sample_importance/message_bit0_sample_importance.npy", sample_importance)



models = load_models(model_folder+"bit_in_byte_7/", model_name)
...
for i, model in enumerate(models):
    ...
    # Save plots
    plt.savefig(f"sample_importance/message_bit7_sample_importance_{i}.png")
...
np.save("sample_importance/message_bit7_sample_importance.npy", sample_importance)
