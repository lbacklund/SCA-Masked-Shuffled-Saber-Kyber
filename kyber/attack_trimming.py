...
##################################################
index_model_folder = "models/index/25k_trim1/"        
message_model_folder = "models/message/25k/"
message_bit0_model_folder = "models/message/25k_trim1/bit_in_byte_0/"        
message_bit7_model_folder = "models/message/25k_trim1/bit_in_byte_7/"        
...
SAVE_STATISTICS_FILE = "attack_statistics_trimmed.txt"
...
def trim_traces(index_traces, message_bit0_traces, message_bit7_traces):
    index_trim_mask = np.load("trim_masks/index_trim_mask.npy")
    message_bit0_trim_mask = np.load("trim_masks/message_bit0_trim_mask.npy")-370
    message_bit7_trim_mask = np.load("trim_masks/message_bit7_trim_mask.npy")-370
    ...
    message_bit0_traces = np.reshape(np.reshape(message_bit0_traces, (message_bit0_traces.shape[0]*message_bit0_traces.shape[1], message_bit0_traces.shape[2]))[:, message_bit0_trim_mask], (message_bit0_traces.shape[0], message_bit0_traces.shape[1], message_bit0_trim_mask.shape[0]))
    message_bit7_traces = np.reshape(np.reshape(message_bit7_traces, (message_bit7_traces.shape[0]*message_bit7_traces.shape[1], message_bit7_traces.shape[2]))[:, message_bit7_trim_mask], (message_bit7_traces.shape[0], message_bit7_traces.shape[1], message_bit7_trim_mask.shape[0]))

    return index_traces, message_bit0_traces, message_bit7_traces
...
def get_predictions(set, models, traces, part, CT_num, k1, k0, used_traces):
    ...
    if traces != None:
        ...
        message_bit0_traces = traces["message_bit0_traces"][CT_num]
        message_bit7_traces = traces["message_bit7_traces"][CT_num]
        ...
        index_traces = None
        message_bit0_traces = None
        message_bit7_traces = None
    ...
    prediction_folder = f"predictions_trimmed/set_{set}/Part{part}_CT{k1}-{k0}/"
    try:
    ...
    if traces != None:
        print("trace shapes:", index_traces.shape, message_bit0_traces.shape, message_bit7_traces.shape)
    ...
    if BIT_IN_BYTE_MODELS:
        if os.path.exists(prediction_folder+"message_bit0.npy"):
            ...
            for m in tqdm(range(len(message_bit0_models)), "Predicting message bits (bit in byte: 0)"):
                ...
                message_bit0_predictions[m] = np.reshape(model_bit0.predict(np.reshape(message_bit0_traces,
                    (message_bit0_traces.shape[0]*message_bit0_traces.shape[1], message_bit0_traces.shape[2]))),
                    (message_bit0_traces.shape[0], message_bit0_traces.shape[1], 2))
        ...
        if os.path.exists(prediction_folder+"message_bit7.npy"):
            ...
            for m in tqdm(range(len(message_bit7_models)), "Predicting message bits (bit in byte: 7)"):
                ...
                message_bit7_predictions[m] = np.reshape(model_bit7.predict(np.reshape(message_bit7_traces,
                    (message_bit7_traces.shape[0]*message_bit7_traces.shape[1], message_bit7_traces.shape[2]))),
                    (message_bit7_traces.shape[0], message_bit7_traces.shape[1], 2))
...
def load_traces(filenum, part):
    ...
    index_len = len(np.load("trim_masks/index_trim_mask.npy"))
    message_bit0_len = len(np.load("trim_masks/message_bit0_trim_mask.npy"))
    message_bit7_len = len(np.load("trim_masks/message_bit7_trim_mask.npy"))

    # Initialize arrays for all traces
    all_index_traces = np.zeros((len(ECC_tool.ct_table), 128*REPS, 253, index_len), dtype=np.float32)
    all_message_bit0_traces = np.zeros((len(ECC_tool.ct_table),128*REPS, 254, message_bit0_len), dtype=np.float32)
    all_message_bit7_traces = np.zeros((len(ECC_tool.ct_table),128*REPS, 254, message_bit7_len), dtype=np.float32)
    ...
    for CT_num, (k1, k0, k2) in enumerate(ECC_tool.ct_table):
        ...
        message_bit0_traces = message_traces
        message_bit7_traces = message_traces.copy()

        # Trim traces
        index_traces, message_bit0_traces, message_bit7_traces = trim_traces(index_traces, message_bit0_traces, message_bit7_traces)

        # Save cut traces to array
        all_index_traces[CT_num] = index_traces
        all_message_bit0_traces[CT_num] = message_bit0_traces
        all_message_bit7_traces[CT_num] = message_bit7_traces
    ...
    message_bit0_traces = standardize_traces(all_message_bit0_traces)
    message_bit7_traces = standardize_traces(all_message_bit7_traces)
    ...
    traces["message_bit0_traces"] = message_bit0_traces
    traces["message_bit7_traces"] = message_bit7_traces
...
def attack(use_traces, set=None):
    ...
    for part in range(3):
        ...
        if os.path.exists(f"predictions_trimmed/set_{set}/Part{part}_CT{ECC_tool.ct_table[-1][0]}-{ECC_tool.ct_table[-1][1]}/"):
            models = None
            traces = None
        else:
            # Load models
...
def main():
    ...
    if len(sys.argv) == 2:
        set = int(sys.argv[1])
        data_folder = data_folder_prefix+f"cd8/set_{set}/"
