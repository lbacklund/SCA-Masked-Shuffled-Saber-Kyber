...
model_folder = "models/index/25k_trim1/"
modelName = 'kyber_index_'
number_of_traces = 25000
#################################################
...
def load_traces():
    ...
    trimmask = np.load("index_trim_mask.npy")

    untrimmed_traces = np.load(data_folder+'cut_joined_traces.npy', mmap_mode="r")[:number_of_traces*253]
    labels = np.load(data_folder+'cut_joined_shuffle_labels.npy', mmap_mode="r")[:number_of_traces*253]
    traces = np.empty((number_of_traces*253, len(trimmask)))

    print("Trimming using mask:", trimmask)
    for i in trange(number_of_traces*253):
...
def train_model(X_profiling, Y_profiling, model, save_file_name, i, epochs=100, batch_size=32, patience=50):  
    ...
    plt.savefig('history/index_trim/' +modelName+str(i)+'acc.pdf')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig('history/index_trim/' +modelName+str(i)+'loss.pdf')
...
for model_name in os.listdir(model_folder):
    if fnmatch.fnmatch(model_name, "kyber_index_[0-9]*.h5"):
...
for i in range(model_number, 10):
    ...
    train_model(traces, labels, mlp, model_folder + modelName, i, epochs=500, batch_size=2048, patience=30)
