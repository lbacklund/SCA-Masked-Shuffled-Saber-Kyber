...
model_folder = "models/message_beginning/25k/"
modelName = 'kyber_message_beginning_'
#################################################
...
def load_traces():
    ...
    traces = np.load(data_folder+'cut_joined_traces_beginning.npy', mmap_mode="r")[:25000, -225:]
    labels = np.load(data_folder+'cut_joined_message_labels_beginning.npy', mmap_mode="r")[:25000]
...
def train_model(X_profiling, Y_profiling, model, save_file_name, i, epochs=100, batch_size=32, patience=50, classes=2):  
    ...
    plt.savefig('history/message_beginning/' +modelName+str(i)+'acc.pdf')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig('history/message_beginning/' +modelName+str(i)+'loss.pdf')
...
for model_name in os.listdir(model_folder):
    if fnmatch.fnmatch(model_name, "kyber_message_beginning_[0-9]*.h5"):
        n = int(model_name[24:-3])
...
for i in range(model_number,10):  
    ...
    train_model(traces, labels, mlp, model_folder + modelName, i, epochs=500, batch_size=32, patience=100, classes=2)
