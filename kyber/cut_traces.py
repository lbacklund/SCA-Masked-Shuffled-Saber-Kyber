...
def load_traces(file_number, plot=False):
    ...
    traces = np.empty((253*shuffle_labels_all.shape[0], shuffle_seg_size+3*message_seg_size))
    shuffle_labels = np.empty((253*shuffle_labels_all.shape[0],))
    message_labels = np.empty((253*message_labels_all.shape[0],))

    #traces = shuffle_traces_all[:, :shuffle_seg_size]
    #traces = np.append(traces, message_traces_all[:, message_offset+message_seg_size*255:message_offset+message_seg_size*256], axis=1)

    #trace_avg = traces.mean(axis=0) # mean of columns
    #plt.plot(trace_avg) 

    #labels = shuffle_labels_all[:,255] # array of indexes of 0th bit of all traces
    ...
    for k in range(1,254):
        traces[traces_per_byte*(k-1):traces_per_byte*k, :shuffle_seg_size] = shuffle_traces_all[:,shuffle_seg_size*k:shuffle_seg_size*(k+1)]
        traces[traces_per_byte*(k-1):traces_per_byte*k, shuffle_seg_size:] = message_traces_all[:,message_seg_size*(254-k):message_seg_size*(257-k)]

        shuffle_labels[traces_per_byte*(k-1):traces_per_byte*k] = shuffle_labels_all[:,255-k]
        message_labels[traces_per_byte*(k-1):traces_per_byte*k] = labels_bits[:, 255-k]
...
def main():
    if len(sys.argv) >= 2:
        ...
        traces, shuffle_labels, message_labels = load_traces(file_number)
        np.save(TRACE_FOLDER+"cut_traces_"+str(file_number), traces)
        np.save(TRACE_FOLDER+"cut_shuffle_labels_"+str(file_number), shuffle_labels)
        np.save(TRACE_FOLDER+"cut_message_labels_"+str(file_number), message_labels)
    else:
