...
def load_traces(file_number, plot=False):
    ...
    traces = np.empty((shuffle_labels_all.shape[0], shuffle_seg_size+3*message_seg_size))
    ...
    for k in range(254,255):
        traces[:, :shuffle_seg_size] = shuffle_traces_all[:, shuffle_seg_size*k:shuffle_seg_size*(k+1)]
        traces[:, shuffle_seg_size:] = message_traces_all[:, message_seg_size*(254-k):message_seg_size*(257-k)]

        shuffle_labels[:] = shuffle_labels_all[:,255-k]
        message_labels[:] = labels_bits[:, 0]
...
def main():
    if len(sys.argv) >= 2:
        ...
        np.save(TRACE_FOLDER+"cut_traces_end_"+str(file_number), traces)
        np.save(TRACE_FOLDER+"cut_shuffle_labels_end_"+str(file_number), shuffle_labels)
        np.save(TRACE_FOLDER+"cut_message_labels_end_"+str(file_number), message_labels)
    else:
