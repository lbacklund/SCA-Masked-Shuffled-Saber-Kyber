...
model_folder = "models/message_end/25k/"
modelName = 'kyber_message_end_'
#################################################
...
def load_traces():
    ...
    traces = np.load(data_folder+'cut_joined_traces_end.npy', mmap_mode="r")[:25000, -3*225:-2*225]
    labels = np.load(data_folder+'cut_joined_message_labels_end.npy', mmap_mode="r")[:25000]
...
def train_model(X_profiling, Y_profiling, model, save_file_name, i, epochs=100, batch_size=32, patience=50, classes=2):  
    ...
    plt.savefig('history/message_end/' +modelName+str(i)+'acc.pdf')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig('history/message_end/' +modelName+str(i)+'loss.pdf')
...
for model_name in os.listdir(model_folder):
    if fnmatch.fnmatch(model_name, "kyber_message_end_[0-9]*.h5"):
        n = int(model_name[18:-3])
