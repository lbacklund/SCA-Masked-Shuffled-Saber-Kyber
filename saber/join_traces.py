...
def map_file_trim(params):
    ...
    traces = np.load(SOURCE_FOLDER+"cut_traces_"+str(file_number)+".npy", mmap_mode="r")
    shuffle_labels = np.load(SOURCE_FOLDER+"cut_shuffle_labels_"+str(file_number)+".npy", mmap_mode="r")
    message_labels = np.load(SOURCE_FOLDER+"cut_message_labels_"+str(file_number)+".npy", mmap_mode="r")
...
def join_trim(file_numbers, index_trim_mask, message_trim_mask):
    ...
    final_shuffle_traces = np.memmap(TARGET_FOLDER+"cut_joined_shuffle_traces.npy", shape=(5000*253*len(file_numbers), len(index_trim_mask)), offset=128, dtype='float64', mode="w+")
    header = np.lib.format.header_data_from_array_1_0(final_shuffle_traces)
    with open(TARGET_FOLDER+"cut_joined_shuffle_traces.npy", 'r+b') as f:
        np.lib.format.write_array_header_1_0(f, header)
    
    final_message_traces = np.memmap(TARGET_FOLDER+"cut_joined_message_traces.npy", shape=(5000*253*len(file_numbers), len(message_trim_mask)), offset=128, dtype='float64', mode="w+")
    header = np.lib.format.header_data_from_array_1_0(final_message_traces)
    with open(TARGET_FOLDER+"cut_joined_message_traces.npy", 'r+b') as f:
        np.lib.format.write_array_header_1_0(f, header)
    
    final_shuffle_labels = np.memmap(TARGET_FOLDER+"cut_joined_shuffle_labels.npy", shape=(5000*253*len(file_numbers),), offset=128, dtype='int', mode="w+")
    header = np.lib.format.header_data_from_array_1_0(final_shuffle_labels)
    with open(TARGET_FOLDER+"cut_joined_shuffle_labels.npy", 'r+b') as f:
        np.lib.format.write_array_header_1_0(f, header)

    final_message_labels = np.memmap(TARGET_FOLDER+"cut_joined_message_labels.npy", shape=(5000*253*len(file_numbers),), offset=128, dtype='int', mode="w+")
    header = np.lib.format.header_data_from_array_1_0(final_message_labels)
    with open(TARGET_FOLDER+"cut_joined_message_labels.npy", 'r+b') as f:
        np.lib.format.write_array_header_1_0(f, header)
    
    print("Creating permutation of length", len(final_shuffle_traces))
    permutation = np.random.permutation(len(final_shuffle_traces))
    np.save(TARGET_FOLDER+"join_permutation.npy", permutation)
...
def map_file(params):
    ...
    traces = np.load(SOURCE_FOLDER+"cut_traces_"+str(file_number)+".npy", mmap_mode="r")
    shuffle_labels = np.load(trace_folder+"cut_shuffle_labels_"+str(file_number)+".npy", mmap_mode="r")
    message_labels = np.load(trace_folder+"cut_message_labels_"+str(file_number)+".npy", mmap_mode="r")
    for n in tqdm(range(5000*253), "Mapping file "+str(file_number)):
...
def join(file_numbers):
    ...
    trace_len = np.load(SOURCE_FOLDER+"cut_traces_0.npy", mmap_mode="r").shape[1]
    final_traces = np.memmap(TARGET_FOLDER+"cut_joined_traces.npy", shape=(5000*253*len(file_numbers), trace_len), offset=128, dtype='float64', mode="w+")
    header = np.lib.format.header_data_from_array_1_0(final_traces)
    with open(TARGET_FOLDER+"cut_joined_traces.npy", 'r+b') as f:
        np.lib.format.write_array_header_1_0(f, header)
