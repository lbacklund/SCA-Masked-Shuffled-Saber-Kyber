...
model_folder = "models/index_end/15k/"
modelName = 'saber_index_end_'
#################################################
...
def load_traces():
    ...
    traces = np.load(data_folder+'cut_joined_traces_end.npy', mmap_mode="r")[:15000,:]
    labels = np.load(data_folder+'cut_joined_shuffle_labels_end.npy', mmap_mode="r")[:15000]
...
def train_model(X_profiling, Y_profiling, model, save_file_name, i, epochs=100, batch_size=32, patience=50):  
    ...
    plt.savefig('history/index_end/' +modelName+str(i)+'acc.pdf')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig('history/index_end/' +modelName+str(i)+'loss.pdf')
...
for model_name in os.listdir(model_folder):
    if fnmatch.fnmatch(model_name, "saber_index_end_[0-9]*.h5"):
        n = int(model_name[16:-3])
